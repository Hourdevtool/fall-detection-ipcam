...
> **ผู้พัฒนา**: AI Assistant (Antigravity)  
...
ใช้ **SQLite** ในโหมด **WAL** (Write-Ahead Logging) สำหรับ concurrent reads ที่ดีขึ้น  
...
```
1. API Server เริ่มทำงาน → สร้างรหัส 6 หลัก (เช่น 043615)
2. รหัสมีอายุ 10 นาที → แสดงในหน้าจอ desktop หรือเรียก GET /api/pair/current
3. ผู้ใช้ login ด้วย Google → เข้าหน้า /pair
4. กรอกรหัส 6 หลัก → POST /api/pair
5. Server ตรวจสอบ: ตรงไหม? หมดอายุหรือยัง?
6. ✅ สำเร็จ → บันทึกลง SQLite → is_active = 1
7. ครั้งต่อไป login → เช็ค GET /api/pair/status → is_paired = true → ข้ามไปหน้า Dashboard
```
...
| 4 | **Push Notification** | แจ้งเตือนผ่าน LINE Notify หรือ Web Push เมื่อตรวจพบการล้ม |
| 5 | **Cloudflare Tunnel** | เข้าถึงจากนอกบ้านโดยไม่ต้อง port forwarding |
| 6 | **หลาย System** | รองรับผู้ใช้ 1 คน pair กับหลายระบบ (หลายบ้าน) |
